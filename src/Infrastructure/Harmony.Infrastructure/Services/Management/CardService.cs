...
namespace Harmony.Infrastructure.Services.Management
{
    public class CardService : ICardService
    {
        ...
        {
            var card = await _cardRepository.Get(parentCardId);
            ...
            if (!children.Any())
            {
                return await Result<short?>.FailAsync("Card has no subtasks");
            }

            var totalStoryPoints = await childrenQuery.SumAsync(card => card.StoryPoints) ?? 0;

            card.StoryPoints = (short)totalStoryPoints;

            await _cardRepository.Update(card);

            return await Result<short?>.SuccessAsync(card.StoryPoints);
        }
