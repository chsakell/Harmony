    ...
    public class RoleService : IRoleService
    {
        ...
        public async Task<Result<string>> SaveAsync(RoleRequest request)
        {
            if (string.IsNullOrEmpty(request.Id))
            {
                ...
                {
                    return await Result<string>.FailAsync("Similar Role already exists.");
                }

                var response = await _roleManager.CreateAsync(new HarmonyRole(request.Name, request.Description));
